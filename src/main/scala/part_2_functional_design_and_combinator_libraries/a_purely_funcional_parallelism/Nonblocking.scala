import Nonblocking.Par.{parMap, run}
import TimeMeter.timed
import part_2_functional_design_and_combinator_libraries.a_purely_funcional_parallelism.Actor
...
object P extends App {
  val xs = List.range(1, 1)
  val f = (x: Int) => math.sqrt(x)
  val p = parMap(xs)(f)

  val s = timed(
      run(Executors.newFixedThreadPool(2))(p).sum
  )
  println(s)
}
